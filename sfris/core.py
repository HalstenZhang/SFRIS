...
class SFRIS:
    ...
    def _write_isomer_lis(self):
        ...
        eq_ref_label = (
            "DFT" if self.advopt_ref_energy is not None else "xTB"
        )
        ...
        for rank, (min_idx, entry, g) in enumerate(isomer_entries):
            ...
            if best_e is not None and eq_ref is not None:
                delta_e = (best_e - eq_ref) * 627.509
        ...
        for rank, (min_idx, entry, g) in enumerate(isomer_entries):
            ...
            delta_e = (
                (best_e - eq_ref) * 627.509
                if best_e is not None and eq_ref is not None else None
            )
            content += (
